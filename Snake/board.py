...
class Snake:
    ...
    def move(self, action):
        ...
        if len(self.snake_body) >= 1:
            for seg in self.snake_body:
                if self.snake_head_x == seg[0] and self.snake_head_y == seg[1]:
                    print("is dead due to: Case where it moves into itself when body length greater than 1")
                    return True

        #   Case when it moves into itslef when body lenght is 1
        if len(self.snake_body) == 1:
            if old_body_head == (self.snake_head_x, self.snake_head_y):
                print("is dead due to: Case when it moves into itslef when body lenght is 1")
                return True
        ...
        if (self.snake_head_x < helper.GRID_SIZE or self.snake_head_y < helper.GRID_SIZE or
            ...
            print("is dead due to: dead on hitting wall")
            if (self.snake_head_x < helper.GRID_SIZE):
                print("self.snake_head_x < helper.GRID_SIZE: ", self.snake_head_x, " < ", helper.GRID_SIZE)
            if (self.snake_head_y < helper.GRID_SIZE):
                print("self.snake_head_y < helper.GRID_SIZE: ", self.snake_head_y, " < ", helper.GRID_SIZE)
            if (self.snake_head_x + helper.GRID_SIZE > helper.DISPLAY_SIZE-helper.GRID_SIZE):
                print("self.snake_head_x + helper.GRID_SIZE > helper.DISPLAY_SIZE-helper.GRID_SIZE: ", self.snake_head_x + helper.GRID_SIZE, " > ", helper.DISPLAY_SIZE-helper.GRID_SIZE)
            if (self.snake_head_y + helper.GRID_SIZE > helper.DISPLAY_SIZE-helper.GRID_SIZE): 
                print("self.snake_head_y + helper.GRID_SIZE > helper.DISPLAY_SIZE-helper.GRID_SIZE: ", self.snake_head_y + helper.GRID_SIZE, " > ", helper.DISPLAY_SIZE-helper.GRID_SIZE)
            return True

        #   Starvation case
        if self.steps > self.starve_steps:
            print("is dead due to: Starvation case")
            return True
